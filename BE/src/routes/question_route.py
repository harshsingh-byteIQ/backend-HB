...
from src.services.question_service import create_question, delete_respective_question , get_all_question , get_all_question_by_role
...
def get_all_questions(db: Session = Depends(get_db)):
    return get_all_question(db)

@QuestionRoute.get("/get-question-by-role/{role}", response_model=list[QuestionOut])
def get_all_questions(role = str , db: Session = Depends(get_db)):
    return get_all_question_by_role(role, db)